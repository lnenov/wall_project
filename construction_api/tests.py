...
class ModelTests(TestCase):
    ...
    def test_create_wall_section(self):
        ...
        section = WallSection.objects.create(profile=profile, section_index=0, initial_height=20, current_height=20)
        self.assertFalse(section.is_complete)
        self.assertIn("20/30", str(section))
        section_complete = WallSection.objects.create(profile=profile, section_index=1, initial_height=30, current_height=30, is_complete=True)
    ...
    def test_wall_section_unique_together(self):
        profile = WallProfile.objects.create(original_index=1)
        WallSection.objects.create(profile=profile, section_index=0, initial_height=10, current_height=10)
        with self.assertRaises(Exception):
             WallSection.objects.create(profile=profile, section_index=0, initial_height=15, current_height=15)

    def test_create_daily_record(self):
        profile = WallProfile.objects.create(original_index=1)
        record = DailyRecord.objects.create(profile=profile, day=1, is_overall=False, ice_yards_today=195, cost_today=COST_PER_FOOT, cumulative_cost=COST_PER_FOOT)
        self.assertEqual(DailyRecord.objects.count(), 1)
        self.assertIn("Profile 1", str(record))
        overall_record = DailyRecord.objects.create(profile=None, day=1, is_overall=True, ice_yards_today=585, cost_today=COST_PER_FOOT*3, cumulative_cost=COST_PER_FOOT*3)
    ...
    def test_create_simulation_metadata(self):
        ...
        meta = SimulationMetadata.objects.create(total_days=15, final_overall_cost=10000000)
...
class RunSimulationCommandTests(TransactionTestCase): # Use TransactionTestCase for command tests

    def setUp(self):
        self.temp_config_file = tempfile.NamedTemporaryFile(mode='w', delete=False, suffix=".txt")
    ...
    def test_command_clears_previous_data(self):
        ...
        WallSection.objects.create(profile=p, section_index=0, initial_height=10, current_height=10)
        DailyRecord.objects.create(profile=p, day=1, cost_today=100, cumulative_cost=100)
    ...
    def test_command_simulation_produces_results(self):
        ...
        self.assertEqual(meta.final_overall_cost, 2 * COST_PER_FOOT) # 2 sections * 1 foot * cost/foot

        # Verify specific daily records
        self.assertEqual(DailyRecord.objects.count(), 3) # 2 profile records + 1 overall for Day 1
...
class APIViewTests(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.s1_1 = WallSection.objects.create(profile=cls.p1, section_index=0, initial_height=28, current_height=28)
        cls.s1_2 = WallSection.objects.create(profile=cls.p1, section_index=1, initial_height=29, current_height=29)
        cls.s2_1 = WallSection.objects.create(profile=cls.p2, section_index=0, initial_height=29, current_height=29)
        ...
        cls.dr_p1_d1 = DailyRecord.objects.create(profile=cls.p1, day=1, is_overall=False, ice_yards_today=2*YARDS_PER_FOOT, cost_today=cost_d1_p1, cumulative_cost=cost_d1_p1)
        cls.dr_p2_d1 = DailyRecord.objects.create(profile=cls.p2, day=1, is_overall=False, ice_yards_today=1*YARDS_PER_FOOT, cost_today=cost_d1_p2, cumulative_cost=cost_d1_p2)
        cls.dr_ov_d1 = DailyRecord.objects.create(profile=None, day=1, is_overall=True, ice_yards_today=3*YARDS_PER_FOOT, cost_today=cost_d1_ov, cumulative_cost=cost_d1_ov)

        # Day 2: Only section s1_1 works (s1_2 and s2_1 finished after day 1)
        cost_d2_p1 = 1 * COST_PER_FOOT
        cost_d2_ov = 1 * COST_PER_FOOT
        cls.dr_p1_d2 = DailyRecord.objects.create(profile=cls.p1, day=2, is_overall=False, ice_yards_today=1*YARDS_PER_FOOT, cost_today=cost_d2_p1, cumulative_cost=cost_d1_p1 + cost_d2_p1)
        # No record for P2 on Day 2
        cls.dr_ov_d2 = DailyRecord.objects.create(profile=None, day=2, is_overall=True, ice_yards_today=1*YARDS_PER_FOOT, cost_today=cost_d2_ov, cumulative_cost=cost_d1_ov + cost_d2_ov)

        # Metadata
        cls.final_cost = cost_d1_ov + cost_d2_ov
        cls.meta = SimulationMetadata.objects.create(total_days=2, final_overall_cost=cls.final_cost)

    def test_daily_ice_view_success(self):
        url = reverse('construction_api:daily_ice', kwargs={'profile_id': 1, 'day_number': 1})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, {'day': 1, 'ice_amount': 2 * YARDS_PER_FOOT}) # P1 worked 2 sections

        url = reverse('construction_api:daily_ice', kwargs={'profile_id': 1, 'day_number': 2})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, {'day': 2, 'ice_amount': 1 * YARDS_PER_FOOT}) # P1 worked 1 section

    def test_daily_ice_view_no_work_on_day(self):
        # Profile 2 did no work on Day 2
        url = reverse('construction_api:daily_ice', kwargs={'profile_id': 2, 'day_number': 2})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, {'day': 2, 'ice_amount': 0})

    def test_daily_ice_view_profile_not_found(self):
        url = reverse('construction_api:daily_ice', kwargs={'profile_id': 999, 'day_number': 1})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_daily_ice_view_day_out_of_range(self):
        url = reverse('construction_api:daily_ice', kwargs={'profile_id': 1, 'day_number': 3}) # Day 3 is beyond simulated days
        response = self.client.get(url)
    ...
    def test_cost_overview_profile_day_success(self):
        ...
        url = reverse('construction_api:profile_cost_overview_day', kwargs={'profile_id': 1, 'day_number': 2})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, {'day': 2, 'cost': self.dr_p1_d2.cumulative_cost})
    ...
    def test_cost_overview_profile_day_no_record_returns_latest(self):
        ...
        url = reverse('construction_api:profile_cost_overview_day', kwargs={'profile_id': 2, 'day_number': 2})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        # Cost should be the cumulative cost from the latest record for P2 on or before Day 2 (which is Day 1)
        self.assertEqual(response.data, {'day': 2, 'cost': self.dr_p2_d1.cumulative_cost})

    def test_cost_overview_overall_day_success(self):
        url = reverse('construction_api:overall_cost_overview_day', kwargs={'day_number': 2})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, {'day': 2, 'cost': self.dr_ov_d2.cumulative_cost})
    ...
    def test_cost_overview_day_out_of_range(self):
        url = reverse('construction_api:overall_cost_overview_day', kwargs={'day_number': 3})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
