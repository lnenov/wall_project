...
from construction_api.models import WallProfile, WallSection, DailyRecord, SimulationMetadata
...
class Command(BaseCommand):
    help = ('Clears previous data, runs the single-threaded wall construction simulation, '
            'and stores results in the database.')

    # Removed --teams argument
    def add_arguments(self, parser):
        parser.add_argument('config_file', type=str, help='Path to the input config file.')
        ...
        try:
                ...
                for line in f:
                    ...
                    try:
                        heights = [int(h) for h in line.split()]
                         ...
                         raise ValueError(f"Non-integer height found in profile line {profile_index_counter}")

                    if not all(0 <= h <= TARGET_HEIGHT for h in heights):
                         raise ValueError(f"Invalid height (0-{TARGET_HEIGHT}) in profile {profile_index_counter}")
                    if len(heights) > 2000:
                         raise ValueError(f"Profile {profile_index_counter} exceeds 2000 sections")
                    ...
                    for section_idx, height in enumerate(heights):
                        sections_to_create.append(WallSection(
            ...
            for section in sections_to_create:
                ...
                if profile_pk:
                    ...
                    raise RuntimeError(f"Could not map profile PK for profile index {section._profile_original_index}")
        ...
        try:
            while True:
                ...
                if not sections_to_update.exists():
                     self.stdout.write(self.style.SUCCESS(f"Day {day}: All sections completed!"))
                     day -= 1 # Simulation finished on the previous day
                     break

                # Get PKs before update for efficient processing
                section_pks_worked_today = list(sections_to_update.values_list('pk', flat=True))
                if not section_pks_worked_today: # Should be caught by exists() but double check
                    self.stdout.write(self.style.WARNING(f"Day {day}: No sections found to update, but exists() was true? Finishing."))
                    day -= 1
                    break

                # 2. Perform the update (+1 foot for all active sections)
                # Use F() expression for atomic increment across all selected sections
                updated_count = sections_to_update.update(current_height=F('current_height') + 1)
                logger.debug(f"Day {day}: Attempted to update {len(section_pks_worked_today)} sections, DB reported {updated_count} updated.")
                if updated_count != len(section_pks_worked_today):
                     logger.warning(f"Day {day}: Mismatch in expected vs actual updated sections ({len(section_pks_worked_today)} vs {updated_count}).")


                # 3. Mark newly completed sections
                newly_completed_pks = list(WallSection.objects.filter(
                ...
                if newly_completed_pks:
                    WallSection.objects.filter(pk__in=newly_completed_pks).update(is_complete=True)
                    logger.debug(f"Day {day}: Marked {len(newly_completed_pks)} sections as complete.")
                ...
                worked_sections_data = WallSection.objects.filter(
                    pk__in=section_pks_worked_today
                ).values('pk', 'profile_id') # Get profile_id associated with each worked section
                ...
                for p_pk, feet_added in feet_added_per_profile_pk.items():
                    if feet_added == 0: continue # Skip profiles where no sections were worked (shouldn't happen here)

                    ice_today = feet_added * YARDS_PER_FOOT
                    cost_today = feet_added * COST_PER_FOOT

                    cumulative_cost_per_profile[p_pk] += cost_today # Update cumulative for this profile PK
                    day_overall_ice += ice_today
                    day_overall_cost += cost_today

                    daily_records_to_create.append(DailyRecord(
                        profile_id=p_pk,
                        day=day,
                        is_overall=False,
                ...
                if day_overall_cost > 0 or day_overall_ice > 0 : # Only create overall if work was done
                    daily_records_to_create.append(DailyRecord(
                        profile_id=None,
                        day=day,
                        is_overall=True,
            ...
            self.stdout.write(self.style.SUCCESS(f"Simulation finished successfully after {final_day} days."))
            self.stdout.write(self.style.SUCCESS(f"Final overall cost: {final_cost}"))
            self.stdout.write(self.style.SUCCESS(f"Total simulation time: {end_time - start_time:.2f} seconds"))
            self.stdout.write(self.style.SUCCESS("Results saved to database and available via API."))
            self.stdout.write("-" * 20)
